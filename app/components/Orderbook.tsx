...
export function Orderbook({ activePair }: { activePair: string }) {
                ...
                {loading && (!rows || rows.length === 0) ? (
                  ...
                  rows.map((row, i) => (
                    ...
                    <TableRow key={i} className="hover:bg-muted/20">
                      ...
                      <TableCell>{row.binanceBid || 0}</TableCell>
                      <TableCell>{row.bybitBid || 0}</TableCell>
                      <TableCell>{row.coinDcxBid || 0}</TableCell> 
                      <TableCell>
                        <AskBar value={row.totalAsk} />
                      </TableCell>
                      <TableCell>{row.binanceAsk || 0}</TableCell>
                      <TableCell>{row.bybitAsk || 0}</TableCell>
                      <TableCell>{row.coinDcxAsk || 0}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
                    <TableCell colSpan={9} className="text-center py-4 text-muted-foreground">
                      No orderbook liquidity depth found.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
